...
rm -rf /etc/nginx/cert.crt
rm -rf /etc/nginx/key.key

# копируем сертификаты из образа certbot в папку Nginx
cp /etc/letsencrypt/live/certfolder*/fullchain.pem /etc/nginx/cert.crt
cp /etc/letsencrypt/live/certfolder*/privkey.pem /etc/nginx/key.key
